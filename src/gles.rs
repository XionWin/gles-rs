...
pub fn check_error() -> c_uint {
    unsafe { crate::ffi::glGetError() }
}

pub fn get_string(name: crate::def::StringName) -> String {
    String::from(
        unsafe { CStr::from_ptr(crate::ffi::glGetString(name)) }
            .to_str()
            .unwrap(),
    )
}
...
pub fn buffer_data(
    ...
    size: c_int,
    data: *const c_void,
    hint: crate::def::BufferUsageHint,
) {
    unsafe {
...
pub fn enable_vertex_attrib_array(index: c_uint) {
    unsafe { crate::ffi::glEnableVertexAttribArray(index) }
}
